...
function longestString(strs) {
  ...
  //reverse array, so that the "first" longest string is returned
  var reverseStrings = strs.slice(0).reverse();
  for (i = 0; i < reverseStrings.length; ++i) {
    if (reverseStrings[i].length >= longest) {
      longest = reverseStrings[i].length;
      longestStr = reverseStrings[i];
    }
  }
  return longestStr;
}
